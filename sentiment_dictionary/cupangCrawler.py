...
def get_product_review(product, num_of_review):
    result = {5: [], 4: [], 3: [], 2: [], 1: []}
    ...
    executable_path='/setiment_dictionary_project/chromedriver', options=options)
    ...
    driver.implicitly_wait(10) # 로딩될 때까지 기다릴 시간
    ...
    for high_review in high_reviews:
        temp = komoran.get_morphes_by_tags(high_review, tag_list=['VA', 'MAG', 'VV']) #형용사, 부사, 동사
        #print(temp)
        high_words += temp
    ...
    print((count_high_words- count_low_words).most_common(15))
    print((count_low_words-count_high_words).most_common(15))

    return result

result = get_product_review("에어팟 맥스", 100)
...
# 리뷰 내용 뿐만 아니라 제목도 크롤링되게 추가 필요
# try-catch로 예외 처리 필요 (리뷰가 더 없다거나 등등..)
# 한 페이지에 모두 리뷰 내용이 없는 경우 멈추는 버그 수정 필요
# 평점별 보기 눌러서 해당하는 1점 / 5점 평점 크롤링하기